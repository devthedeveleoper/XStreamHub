...
const VideoPlayerPage = () => {
  ...
  const fetchSidebarVideos = useCallback(async (currentPage) => {
    ...
    try {
      ...
      setSidebarVideos(prev => currentPage === 1 ? videoData : [...prev, ...videoData]);
      if (response.data.currentPage) {
        setHasMoreSidebarVideos(response.data.currentPage < response.data.totalPages);
      }
    } catch (err) {
  ...
  }, [inView, hasMoreSidebarVideos, loadingSidebar, sidebarPage, fetchSidebarVideos]);
